...
ARG PYPI_URL
ARG PYPI_HOST
ARG PACKAGES="make unzip wget curl jq python-apt python python3-pip apt-transport-https ca-certificates git"
...
RUN pip_args="" ; [ -z "$PYPI_URL" ] || pip_args=" --index-url $PYPI_URL " ; \
    ...
    pip3 install $pip_args -I --no-deps -r /tmp/requirements.txt
